...
f_data_sm %>%
  filter(SEASON == 2019, COUNTRY == "England") %>%
  teams_ha_points()
...
team_ha_adv_filtered <- function(data = f_data_sm, country, season) {
  result <- data %>%
    filter(COUNTRY == "Netherlands", SEASON == "2019") %>%
    teams_ha_points()
...
ered_home_adv_by_teams <- team_ha_adv_filtered(country = "Netherlands",
                                               season = 2019)
